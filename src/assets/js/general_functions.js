...
((window,$,EALang) => {
    ...
    class GeneralFunctions {
        ...
        static displayMessageBox(title = "<No Title Given>", message="<No Message Given>", messageButtons = undefined) {
            ...
            $(GeneralFunctions.MESSAGE_BOX_SELECTOR).dialog({
                autoOpen: false,
                modal: true,
                resize: 'auto',
                width: 'auto',
            ...
            $(GeneralFunctions.MESSAGE_BOX_SELECTOR).dialog('open');
            $('.ui-dialog .ui-dialog-buttonset button').addClass('btn btn-default');
            $(GeneralFunctions.MESSAGE_BOX_SELECTOR + '.ui-dialog-titlebar-close').hide();
        }
        ...
        static centerElementOnPage (elementHandle) {
            ...
            $(window).resize(() => {
                let elementLeft = ($(window).width() - elementHandle.outerWidth()) / 2;
                let elementTop = ($(window).height() - elementHandle.outerHeight()) / 2;
         ...
         */
        static getUrlParameter (url, parameterName) {
            let parsedUrl = url.substr(url.indexOf('?')).slice(1).split('&');

            for (let index in parsedUrl) {
                let parsedValue = parsedUrl[index].split('=');
        ...
        static ISODateString (date) {
            return date.getUTCFullYear() + '-'
                + GeneralFunctions.pad(date.getUTCMonth() + 1) + '-'
                + GeneralFunctions.pad(date.getUTCDate()) + 'T'
                + GeneralFunctions.pad(date.getUTCHours()) + ':'
                + GeneralFunctions.pad(date.getUTCMinutes()) + ':'
                + GeneralFunctions.pad(date.getUTCSeconds()) + 'Z';
        }
        ...
        static clone (originalObject) {
            ...
            if (originalObject instanceof Array) {
                let copy = [];
                for (let i = 0, len = originalObject.length; i < len; i++) {
            ...
            if (originalObject instanceof Object) {
                let copy = {};
                for (let attr in originalObject) {
         ...
         */
        static exceptionsToHtml (exceptions) {
            let html =
            ...
            $.each(exceptions, (index, exception) => {
                html +=
         ...
         */
        static parseExceptions (exceptions) {
         ...
         */
        static handleAjaxExceptions (response) {
            if (response.exceptions) {
                response.exceptions = GeneralFunctions.parseExceptions(response.exceptions);
                GeneralFunctions.displayMessageBox(GeneralFunctions.EXCEPTIONS_TITLE, GeneralFunctions.EXCEPTIONS_MESSAGE);
                $(GeneralFunctions.MESSAGE_BOX_SELECTOR).append(GeneralFunctions.exceptionsToHtml(response.exceptions));
                return false;
            }
            ...
            if (response.warnings) {
                ...
                $(GeneralFunctions.MESSAGE_BOX_SELECTOR).append(GeneralFunctions.exceptionsToHtml(response.warnings));
            }
         ...
         * language selection button. This method requires the global letiable 'availableLanguages'
        ...
        static enableLanguageSelection ($element) {
            ...
            $element.click(() => {
                if ($(GeneralFunctions.LANGUAGE_LIST_SELECTOR).length === 0) {
                    $(this).popover('show');
                } else {
                    $(this).popover('hide');
                }
            ...
            $(document).on('click', 'li.language', () => {
                // Change language with ajax call and refresh page.
                let postUrl = Globalletiables.baseUrl + '/index.php/backend_api/ajax_change_language';
                let postData = {
                    csrfToken: Globalletiables.csrfToken,
                    language: $(this).attr('data-language'),
                };
                $.post(postUrl, postData, (response) => {
         ...
         */
        static ajaxFailureHandler (jqxhr, textStatus, errorThrown) {
            let exceptions = [
                {
            ...
            GeneralFunctions.displayMessageBox(GeneralFunctions.EXCEPTIONS_TITLE, GeneralFunctions.EXCEPTIONS_MESSAGE);
            $(GeneralFunctions.MESSAGE_BOX_SELECTOR).append(GeneralFunctions.exceptionsToHtml(exceptions));
        }
