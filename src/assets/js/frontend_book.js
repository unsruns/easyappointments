 ...
 * @module FrontendBook
 */
(function(window,$, EALang,FrontendBookApi,GeneralFunctions,GlobalVariables) {
    ...
    class FrontendBook {
         ...
         * @param manageMode
         */
        constructor(bindEventHandlers, manageMode){
            /**
             * Determines the functionality of the page.
             *
             * @type {Boolean}
             */
            this.manageMode = manageMode || false;

            this.bindEventHandlers = bindEventHandlers || true;
            ...
            if (this.manageMode) {
                this._applyAppointmentData(GlobalVariables.appointmentData,
                    GlobalVariables.providerData, GlobalVariables.customerData);
            } else {
                let $selectProvider     =   $('#select-provider');
                let $selectService      =   $('#select-service');

                // Check if a specific service was selected (via URL parameter).
                let selectedServiceId = GeneralFunctions.getUrlParameter(location.href, 'service');
                ...
                let selectedProviderId = GeneralFunctions.getUrlParameter(location.href, 'provider');

                if (selectedProviderId && $selectProvider.find('option[value="' + selectedProviderId + '"]').length === 0) {
                    // Select a service of this provider in order to make the provider available in the select box.
                    for (let index in GlobalVariables.availableProviders) {
                        let provider = GlobalVariables.availableProviders[index];
        ...
        _bindEventHandlers() {
             ...
             */
            $('#select-provider').change((e) => {
                FrontendBookApi.getUnavailableDates($(e.currentTarget).val(), $('#select-service').val(),
             ...
             * become visible.
             */
            $('#select-service').change( (e) => {

                let self    =   e.currentTarget;
                let currServiceId = $('#select-service').val();
                ...
                $.each(GlobalVariables.availableProviders, function (indexProvider, provider) {
                    $.each(provider['services'], function (indexService, serviceId) {
                        ...
                        if (serviceId === currServiceId) {
                            let optionHtml = '<option value="' + provider['id'] + '">'
                ...
                FrontendBookApi.getUnavailableDates($('#select-provider').val(), $(self).val(),
                    $('#select-date').datepicker('getDate').toString('yyyy-MM-dd'));
                this.updateConfirmFrame();
                this._updateServiceDescription($('#select-service').val(), $('#service-description'));
            });
            ...
            $('.button-next').click(function () {
                ...
                let nextTabIndex = parseInt($(this).attr('data-step_index')) + 1;
             ...
             */
            $('.button-back').click(() => {
                let prevTabIndex = parseInt($(this).attr('data-step_index')) - 1;
            ...
            if (this.manageMode) {
                 ...
                 */
                $('#cancel-appointment').click((event) => {
                    let dialogButtons = {};
                    ...
                    $(GeneralFunctions.MESSAGE_BOX_SELECTOR).append('<textarea id="cancel-reason" rows="3"></textarea>');
             ...
             * @param {jQuery.Event} event
             */
            $('#book-appointment-submit').click((event) => {
             ...
             * @param {jQuery.Event} event
             */
            $('.captcha-title small').click((event) => {
                $('.captcha-image').attr('src', GlobalVariables.baseUrl + '/index.php/captcha?' + Date.now());
            });


            $('#select-date').on('mousedown', '.ui-datepicker-calendar td', (event) => {
                setTimeout(() => {
        ...
        updateConfirmFrame() {
            ...
            let selServiceId = $('#select-service').val();
            let servicePrice;
            let serviceCurrency;

            $.each(GlobalVariables.availableServices, (index, service) => {
            ...
            // Customer Details
            let firstName = GeneralFunctions.escapeHtml($('#first-name').val());
            let lastName = GeneralFunctions.escapeHtml($('#last-name').val());
            let phoneNumber = GeneralFunctions.escapeHtml($('#phone-number').val());
            let email = GeneralFunctions.escapeHtml($('#email').val());
            let address = GeneralFunctions.escapeHtml($('#address').val());
            let city = GeneralFunctions.escapeHtml($('#city').val());
            let zipCode = GeneralFunctions.escapeHtml($('#zip-code').val());
            ...
            postData['appointment'] = {
                ...
                end_datetime: this._calcEndDatetime(),
                notes: $('#notes').val(),
            ...
            postData['manage_mode'] = this.manageMode;
        ...
        _calcEndDatetime() {
            ...
            let selServiceDuration = undefined;
            ...
            let startDatetime = $('#select-date').datepicker('getDate').toString('dd-MM-yyyy')
                + ' ' + $('.selected-hour').text();
         ...
         */
        _applyAppointmentData(appointment, provider, customer) {
            try {
                ...
                let appointmentNotes = (appointment['notes'] !== null)
                    ? appointment['notes'] : '';
         ...
         * object as value).
         */
        _updateServiceDescription(serviceId, $div) {
            let html = '';
    ...
    window.FrontendBook =   FrontendBook;

})(window,jQuery,EALang,FrontendBookApi,GeneralFunctions,GlobalVariables);
